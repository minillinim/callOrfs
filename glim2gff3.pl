...
#    glim2gff3.pl
#
#    Convert glimmer output to a gff3 file
#
...
my $GLIM_FILE = openRead($global_options->{'in'});
my $GFF3_FILE = openWrite($global_options->{'out'});
...
sub checkParams {
    ...
    # Compulsory items
    if(!exists $options{'in'} ) { printParamError("No file to parse"); }
