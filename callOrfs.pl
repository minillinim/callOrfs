...
if(exists $global_options->{'glimmer'})
{
    ...
    `g3-from-scratch.csh $global_options->{'in'} $tag`;
    ...
    removeFile($tag.".detail");
    removeFile($tag.".icm");
    removeFile($tag.".longorfs");
    removeFile($tag.".predict");
    removeFile($tag.".train");
...
sub checkParams {
    my @standard_options = ( "help|h+", "in|i:s", "out|o:s", "glimmer|g+" );
    my %options;
    ...
    if(!exists $options{'out'} ) { printParamError ("You MUST supply an output file name"); }
...
=head1 DESCRIPTION
   ...
   Run Prodigal or Glimmer on a set of contigs to create a set of 
   orfs which are ready for further analysis
...
=head1 SYNOPSIS
    ...
    callOrfs.pl -in|i CONTIG_FILE -out|o OUTFILE 

      -in -i CONTIG_FILE           Contig file to parse
      -out -o OUTFILE              File to write ORFs to
